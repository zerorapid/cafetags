...
// Remove Cafe Niloufer completely. Wait, there are multiple matches?
newContent = newContent.replace(/,\s*\{\s*"id":\s*\d+,[\s\S]*?"name":\s*"(Cafe Niloufer|Niloufer Cafe)"[\s\S]*?(?=\},\s*\{|\}\s*\];)\}/g, '');
newContent = newContent.replace(/\{\s*"id":\s*\d+,[\s\S]*?"name":\s*"(Cafe Niloufer|Niloufer Cafe)"[\s\S]*?(?=\},\s*\{|\}\s*\];)\},\s*/g, '');
